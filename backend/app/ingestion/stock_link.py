...
def link_catalog_to_stock(db: Session) -> dict:
    ...
    # Which work ids are file-backed (so a hook pointing at one is a stock hook we may re-derive).
    ...
    for g in groups:
        ...
        if len(scored) > 1 and scored[1][0] >= _MIN_SCORE and scored[1][0] > scored[0][0] - 0.05:
            continue
        best_wid = scored[0][1]
        ...
        # this entry) — so a restore-corrupted hook is corrected but a merely-lower-scored correct one
        # is never clobbered.
        if was is not None and verify._title_score(gt, work_titles.get(was, "")) >= 0.5:
            continue
        g.hooked_work_id = best_wid
        # Roll the hook down to the group's member catalog rows (the per-source index entries).
        db.query(CatalogWork).filter(CatalogWork.group_id == g.id).update(
            {CatalogWork.hooked_work_id: best_wid}, synchronize_session=False)
        if was is None:
            linked += 1
        else:
            fixed += 1
