...
# --- 5b) install the console launchers -------------------------------------
# Tiny wrappers that point a venv entry point at the real database with an
# absolute URL, so the commands work from any directory and share state with the
# web app. One per console script: `shelfcli` (terminal reader) and
# `shelfmanage` (per-title library maintenance).
install_launcher() {
  local name="$1" label="$2" target content user_local
  content="#!/usr/bin/env bash
# Auto-generated by Shelf install.sh — $label.
...
"
  ...
  user_local="$RUN_HOME/.local/bin/$name"
  if [ "$target" != "$user_local" ] && [ -e "$user_local" ]; then
    as_user mkdir -p "$RUN_HOME/.local/bin"
    printf '%s' "$content" | as_user tee "$user_local" >/dev/null
    as_user chmod +x "$user_local"
  fi
  ok "$label installed: run '$name'"
}
for _cmd in "shelfcli:terminal reader" "shelfmanage:library maintenance"; do
  _name="${_cmd%%:*}"; _label="${_cmd#*:}"
  if [ -x "$VENV/bin/$_name" ]; then
    install_launcher "$_name" "$_label"
  else
    warn "$_name entry point missing (backend install incomplete?) — skipping launcher"
  fi
done
